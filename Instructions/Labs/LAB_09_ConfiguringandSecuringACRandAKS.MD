...
1. 在“Cloud Shell”窗格中的 Bash 会话中，运行以下命令以从 Dockerfile 生成映像，并将映像推送到新的 ACR。 

    >**备注**：命令行末尾的尾随句点是必需的。它将当前目录指定为 Dockerfile 的位置。 
...
1. 单击 **“下一步”**：**“身份验证 >”**，在 **“创建 Kubernetes 群集”** 边栏选项卡的 **“身份验证”** 选项卡中，接受默认设置，然后单击 **“下一步：联网 >”**。 

    >**注意**：你正在创建一个群集基础结构服务主体，Kubernetes 群集将使用该主体来管理连接到该群集的云资源。有关更多信息，请访问[使用 Azure Kubernetes 服务的服务主体](https://docs.microsoft.com/zh-cn/azure/aks/kubernetes-service-principal)页。

    >**注意**：你正为 Kubernetes 启用基于角色的访问控制。有关更多信息，请访问[将 Azure Active Directory 与 Azure Kubernetes 服务集成](https://docs.microsoft.com/zh-cn/azure/aks/azure-ad-integration)页。 
...
1. 单击 **“查看 + 创建”**，然后单击 **“创建”**。
    ...
    >**注意**：等待部署完成。该操作需要约 10 分钟。

1. 部署完成后，在 Azure 门户中，在 Azure 门户页面顶部的 **“搜索资源、服务和文档”** 文本框中，键入 **“资源组”**，然后按 **Enter** 键。
...
1. 在“Cloud Shell”窗格的 Bash 会话中，运行以下命令以标识 Azure 容器注册表实例的名称：
    ...
    >**备注**：记录 Azure 容器注册表实例名称。稍后，你将在此任务中使用它。
...
1. 在编辑器窗格中，向下滚动到**第 21 行**并将 **`<ACRUniquename>`** 占位符替换为 ACR 名称。
...
1. 在 Cloud Shell 窗格中的 Bash 会话中，查看输出并将值记录在 External-IP 列中。在下一个步骤中你将用到它们。 
